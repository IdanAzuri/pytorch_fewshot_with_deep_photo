...
def compute_laplacian(img):
    ...
    indices = torch.from_numpy(np.mat([coo.row, coo.col]).transpose())
    return torch.sparse.FloatTensor(indices.type(torch.LongTensor).t(), torch.from_numpy(coo.data.astype(np.float32)), torch.Size(coo.shape))