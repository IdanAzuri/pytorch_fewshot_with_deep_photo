...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std, style_layers, content_layers, style_img, content_img, style_masks, content_masks, device, ):
	"""
	Assumptions:
...
def run_style_transfer(cnn, normalization_mean, normalization_std, style_layers, content_layers, style_img, content_img, input_img, style_masks, content_masks, device, reg=False,
	...
	while step <= num_steps:
		...
		def closure():
			...
			if step % 50 == 0:
				print("step {:>4d}:".format(step), "S: {:.3f} C: {:.3f} R:{:.3f}".format(style_score.item(), content_score.item(), reg_loss if reg else 0), )
